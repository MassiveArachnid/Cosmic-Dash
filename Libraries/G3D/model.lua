...
local function newModel(verts, texture, customformat, translation, rotation, scale)
	...
	if customformat == nil then
		customformat = self.vertexFormat
	end
    ...
    self.mesh = love.graphics.newMesh(customformat, self.verts, "triangles")
   -- self.mesh = love.graphics.newMesh(self.verts, "triangles")
...
-- draw instances of the model
function model:drawInstanced(InstanceShader, InstanceCount)
	InstanceShader:send("modelMatrix", self.matrix)
